...
TRAIN_EPISODE = 5000  # Episode limitation
TEST_EPISODE = 20
STEP = 20000  # Step limitation in an episode
...
def main():
    ...
    for episode in range(TRAIN_EPISODE):
        ...
        for step in range(STEP):
            ...
            state = next_state
            if done:
                # printimproved_('TRAIN:timesteps={}'.format(step))
                break
    ...
    test_evn = gym.wrappers.Monitor(
        env, './videos/improved_DQN_cartpole-v0', force=True)
    ...
    agent.visualize_loss('improved_DQN_cartpole_loss')
    visualize(training_test_rewards, 'improved_DQN_carpole_training_test_rewards.png')
    visualize(test_rewards, 'improved_DQN_carpole_test_rewards.png')
    f = open('./improvedDQN/DQN_cartpole_test_reward.pkl',  'wb')
