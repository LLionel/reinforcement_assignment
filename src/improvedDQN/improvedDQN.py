...
class DQN():
    ...
    def __init__(self,
        ...
        self.train_loss = []
        self.count = 0
        self.model_save_pos = self.env_name+'improved_DQN_model.pkl'
    ...
    def create_Q_network(self):
        ...
        torch.save(self.value_net.state_dict(), self.env_name+'improved_DQN_prime.pkl')
    ...
    def train_Q_network(self):
        ...
        # Step 2: calculate y
        y_batch = self.Q_hat(minibatch, reward_batch, next_state_batch)
    ...
    def Q_hat(self, minibatch, reward_batch, next_state_batch):
        y_batch = []
        self.save_model()
        self.value_net.load_state_dict(torch.load(self.env_name+'improved_DQN_prime.pkl'))
    ...
    def save_model(self):
        torch.save(self.value_net.state_dict(), self.model_save_pos)

    def load_model(self):
        self.value_net.load_state_dict(torch.load(self.model_save_pos))

    
    def visualize_loss(self, pic_name):
        fig, ax = plt.subplots()
        f = open('./improvedDQN/'+pic_name+'.pkl', 'wb')
        P.dump(self.train_loss, f)
        f.close()
